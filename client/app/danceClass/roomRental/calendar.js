...
<div className="bg-white">
    ...
    <div className="flex flex-col md:flex-row mx-auto rounded-lg overflow-hidden mt-12 w-2/3 mb-12 shadow-lg shadow-gray-400">
      <div className="bg-white text-gray-800 p-12 w-5/5">
        <div className="flex items-center text-6xl font-bold justify-between mb-12 mt-4">
          <button onClick={prevMonth} className="p-2 hover:bg-gray-200 rounded" aria-label="Previous Month">
            &lt;
          </button>
          <h2 className="text-4xl font-bold">
            {monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}
          </h2>
          <button onClick={nextMonth} className="p-2 hover:bg-gray-200 rounded" aria-label="Next Month">
            &gt;
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {daysOfWeek.map((day, index) => (
            <div key={index} className="text-center text-2xl font-bold text-gray-600">
              {day}
            </div>
          ))}
        </div>
  ...
  <div className="flex justify-center mb-6">
    <button
      ...
      className={`px-4 py-2 mr-4 border rounded-md transition-all ${selectedRoom === "Room X" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
    ...
    <button
      ...
      className={`px-4 py-2 mr-4 border rounded-md transition-all ${selectedRoom === "Room L" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
        } hover:bg-blue-500 hover:text-white`}
    >
      Room L
    </button>
    <button
      ...
      className={`px-4 py-2 border rounded-md transition-all ${selectedRoom === "Room XL" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
        } hover:bg-blue-500 hover:text-white`}
    >
      Room XL
    </button>
  ...
  <div className={`${className} w-full max-w-4xl mx-auto p-4`}>
    <h2 className="text-xl font-medium mb-2 text-gray-800">{formattedClickedDate}</h2>
      ...
      {timeSlots.map((time) => {
        ...
        return (
          <button
            ...
            disabled={isExcluded}
            className={`py-2 px-3 border rounded-md transition-all ${isExcluded
